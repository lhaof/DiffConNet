...
class Dataset3D(Dataset):
    ...
    def window_image(self, image):
        ...
        normalized = (windowed_array - min_intensity) / (window_width + 1e-8)
    ...
    def __getitem__(self, index):
        ...
        # read npz
        # data_all = np.load(self.root + datafiles["img"], 'r')["data"]
        # image = data_all[:-1][0]

        # label & modal
        if 'BraTS2021' in datafiles["img"]:
            if '0000' in datafiles["img"]:
                modal = torch.tensor([0])
                label = torch.tensor([0])
            elif '0001' in datafiles["img"]:
                modal = torch.tensor([1])
                label = torch.tensor([0])
            elif '0002' in datafiles["img"]:
                modal = torch.tensor([2])
                label = torch.tensor([0])
            elif '0003' in datafiles["img"]:
                modal = torch.tensor([3])
                label = torch.tensor([0])
            else:
                print(datafiles['img'])
                print('*************error********')
        elif 'GLI' in datafiles["img"]:
            if 't1n' in datafiles["img"]:
                modal = torch.tensor([0])
                label = torch.tensor([0])
            elif 't2w' in datafiles["img"]:
                modal = torch.tensor([1])
                label = torch.tensor([0])
            elif 't2f' in datafiles["img"]:
                modal = torch.tensor([2])
                label = torch.tensor([0])
            elif 't1c' in datafiles["img"]:
                modal = torch.tensor([3])
                label = torch.tensor([0])
            else:
                print(datafiles['img'])
                print('*************error********')
        elif 'MEN' in datafiles["img"]:
            if 't1n' in datafiles["img"]:
                modal = torch.tensor([0])
        ...
        # image_crop_ori = self.window_image(image_crop_ori)
