...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, c=None, dims=None, modal_type=None):
        if c is None:
            x = x + self.attn(self.norm1(x), dims, modal_type)
            x = x + self.mlp(self.norm2(x))
        else:
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.adaLN_modulation(c).chunk(6, dim=1)
            x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa), dims, modal_type)
