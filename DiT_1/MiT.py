...
import logging
import math
from DiT_1 import MiT_encoder as MiT_encoder
...
class MiT(nn.Module):
    ...
    def __init__(self, norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
        ...
        if modal_type == '2D':
            ...
            self.DecEmbed2D3 = PatchEmbed2D(img_size=img_size2D // 2, patch_size=2, in_chans=embed_dims[2],
                                            embed_dim=32)
            self.DecEmbed2D4 = PatchEmbed2D(img_size=img_size2D, patch_size=2, in_chans=32,
                                            embed_dim=4)
            ...
            # 3D
            self.upsamplex122 = nn.Upsample(scale_factor=(2, 2, 2), mode='trilinear')
            self.DecEmbed3D0 = PatchEmbed3D(img_size=[img_size3D[0] // 16, img_size3D[1] // 16, img_size3D[2] // 16],
                                            patch_size=[2, 2, 2], in_chans=self.transformer.embed_dims[-1], embed_dim=embed_dims[0])
            self.DecEmbed3D1 = PatchEmbed3D(img_size=[img_size3D[0] // 8, img_size3D[1] // 8, img_size3D[2] // 8],
                                            patch_size=[2, 2, 2], in_chans=embed_dims[0], embed_dim=embed_dims[1])
            self.DecEmbed3D2 = PatchEmbed3D(img_size=[img_size3D[0] // 4, img_size3D[1] // 4, img_size3D[2] // 4],
                                            patch_size=[2, 2, 2], in_chans=embed_dims[1], embed_dim=embed_dims[2])
        ...
        # self.head = Head(self.embed_dim, 65536, use_bn=False, norm_last_layer=True)
    ...
    def forward2D(self, inputs, modal_type, t, m, y):
        B = inputs.shape[0]

        ####### encoder
        x_encoder, (H, W) = self.transformer(inputs, modal_type, t, m, y)
        ...
        x, (H, W) = self.DecEmbed2D4(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        return x

    def forward3D(self, inputs, modal_type, t, m, y):
        B = inputs.shape[0]

        ####### encoder
        x_encoder, (D, H, W) = self.transformer(inputs, modal_type, t, m, y)
        cls_tokens = x_encoder[-1][:, 0]
    ...
    def forward(self, inputs, t, m, y):
        # print('inputs.shape',inputs.shape)
        if len(inputs.shape) == 4:
            modal_type = '2D'
            return self.forward2D(inputs, modal_type, t, m, y)
        elif len(inputs.shape) == 5:
            modal_type = '3D'
            return self.forward3D(inputs, modal_type, t, m, y)
