...
# import Pacth_embeds as CNN_base
from DiT_1 import Pacth_embeds as CNN_base

from utils import trunc_normal_

from DiT_1.DiT_models import DiTBlock, TimestepEmbedder, ModalEmbedder, LabelEmbedder
...
class MiT_encoder(nn.Module):
    ...
    def __init__(self, norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False, img_size2D=224,
                 ...
                 attn_drop_rate=0., drop_path_rate=0., depths=[2, 4, 6], sr_ratios=[1, 1, 1],
                 modal_type='MM', num_classes=4, class_dropout_prob=0.1):

        super().__init__()

        self.embed_dims = embed_dims

        self.t_embedder0 = TimestepEmbedder(embed_dims[0])
        self.t_embedder1 = TimestepEmbedder(embed_dims[1])
        self.t_embedder2 = TimestepEmbedder(embed_dims[2])
        self.t_embedder3 = TimestepEmbedder(embed_dims[3])

        self.y_embedder0 = LabelEmbedder(num_classes, embed_dims[0], class_dropout_prob)
        self.y_embedder1 = LabelEmbedder(num_classes, embed_dims[1], class_dropout_prob)
        self.y_embedder2 = LabelEmbedder(num_classes, embed_dims[2], class_dropout_prob)
        self.y_embedder3 = LabelEmbedder(num_classes, embed_dims[3], class_dropout_prob)

        self.m_embedder0 = ModalEmbedder(num_classes, embed_dims[0], class_dropout_prob)
        self.m_embedder1 = ModalEmbedder(num_classes, embed_dims[1], class_dropout_prob)
        self.m_embedder2 = ModalEmbedder(num_classes, embed_dims[2], class_dropout_prob)
        self.m_embedder3 = ModalEmbedder(num_classes, embed_dims[3], class_dropout_prob)



        # Encoder patchEmbed
        if modal_type == '2D':
            self.patch_embed2D0 = CNN_base.Conv2dBlock(1, 32, norm_cfg=norm_cfg2D, activation_cfg=activation_cfg,
            ...
            # 2D
            self.patch_embed2D0 = CNN_base.Conv2dBlock(2, 32, norm_cfg=norm_cfg2D, activation_cfg=activation_cfg,
                                                       weight_std=weight_std, kernel_size=7, stride=2, padding=3)
            ...
            # 3D
            self.patch_embed3D0 = CNN_base.Conv3dBlock(in_chans, 32, norm_cfg=norm_cfg3D, activation_cfg=activation_cfg,
                                                       weight_std=weight_std, kernel_size=7, stride=(2, 2, 2),
                                                       padding=3)

            self.patch_embed3D1 = PatchEmbed3D(norm_cfg=norm_cfg3D, activation_cfg=activation_cfg,
                                               weight_std=weight_std,
                                               img_size=[img_size3D[0] // 2, img_size3D[1] // 2, img_size3D[2] // 2],
                                               patch_size=[2, 2, 2], in_chans=32, embed_dim=embed_dims[0])
            self.patch_embed3D2 = PatchEmbed3D(norm_cfg=norm_cfg3D, activation_cfg=activation_cfg,
                                               weight_std=weight_std,
                                               img_size=[img_size3D[0] // 4, img_size3D[1] // 4, img_size3D[2] // 4],
                                               patch_size=[2, 2, 2], in_chans=embed_dims[0], embed_dim=embed_dims[1])
            self.patch_embed3D3 = PatchEmbed3D(norm_cfg=norm_cfg3D, activation_cfg=activation_cfg,
                                               weight_std=weight_std,
                                               img_size=[img_size3D[0] // 8, img_size3D[1] // 8, img_size3D[2] // 8],
                                               patch_size=[2, 2, 2], in_chans=embed_dims[1], embed_dim=embed_dims[2])
            self.patch_embed3D4 = PatchEmbed3D(norm_cfg=norm_cfg3D, activation_cfg=activation_cfg,
                                               weight_std=weight_std,
                                               img_size=[img_size3D[0] // 16, img_size3D[1] // 16, img_size3D[2] // 16],
                                               patch_size=[2, 2, 2], in_chans=embed_dims[2], embed_dim=embed_dims[3])
        ...
        self.block1 = nn.ModuleList([DiTBlock(
        ...
        self.block2 = nn.ModuleList([DiTBlock(
        ...
        self.block3 = nn.ModuleList([DiTBlock(
        ...
        self.block4 = nn.ModuleList([DiTBlock(
            hidden_size=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3],
            # drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
    ...
    def _init_weights(self, m):
        ...
        nn.init.normal_(self.y_embedder0.embedding_table.weight, std=0.02)
        nn.init.normal_(self.y_embedder1.embedding_table.weight, std=0.02)
        nn.init.normal_(self.y_embedder2.embedding_table.weight, std=0.02)
        nn.init.normal_(self.y_embedder3.embedding_table.weight, std=0.02)

        nn.init.normal_(self.m_embedder0.embedding_table.weight, std=0.02)
        nn.init.normal_(self.m_embedder1.embedding_table.weight, std=0.02)
        nn.init.normal_(self.m_embedder2.embedding_table.weight, std=0.02)
        nn.init.normal_(self.m_embedder3.embedding_table.weight, std=0.02)

        # Initialize timestep embedding MLP:
        nn.init.normal_(self.t_embedder0.mlp[0].weight, std=0.02)
        nn.init.normal_(self.t_embedder0.mlp[2].weight, std=0.02)

        nn.init.normal_(self.t_embedder1.mlp[0].weight, std=0.02)
        nn.init.normal_(self.t_embedder1.mlp[2].weight, std=0.02)

        nn.init.normal_(self.t_embedder2.mlp[0].weight, std=0.02)
        nn.init.normal_(self.t_embedder2.mlp[2].weight, std=0.02)

        nn.init.normal_(self.t_embedder3.mlp[0].weight, std=0.02)
        nn.init.normal_(self.t_embedder3.mlp[2].weight, std=0.02)
    ...
    def forward2D(self, x, t, m, y):
        ...
        # stage 1
        t0 = self.t_embedder0(t)
        print('t0',t0.shape)
        y0 = self.y_embedder0(y, self.training)
        print('y0',y0.shape)
        m0 = self.m_embedder0(m, self.training)
        print('m0',m0.shape)
        c = t0 + m0 + y0
        ...
        for blk in self.block1:
            x = blk(x, c=c, dims=(H, W), modal_type='2D')
        cls_tokens = x[:, 0]
        x = x[:, 1::].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        out.append(x)

        # stage 2
        t1 = self.t_embedder1(t)
        y1 = self.y_embedder1(y, self.training)
        m1 = self.m_embedder1(m, self.training)
        c1 = t1 + m1 + y1
        ...
        for blk in self.block2:
            x = blk(x, c=c1, dims=(H, W), modal_type='2D')
        cls_tokens = x[:, 0]
        x = x[:, 1::].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        out.append(x)
        ...
        y2 = self.y_embedder2(y, self.training)
        m2 = self.m_embedder2(m, self.training)
        c2 = t2 + m2 + y2
        ...
        for blk in self.block3:
            x = blk(x, c=c2, dims=(H, W), modal_type='2D')
        cls_tokens = x[:, 0]
        x = x[:, 1::].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        out.append(x)

        # stage 4
        t3 = self.t_embedder3(t)
        y3 = self.y_embedder3(y, self.training)
        m3 = self.m_embedder3(m, self.training)
        c3 = t3 + m3 + y3
        ...
        for blk in self.block4:
            x = blk(x, c=c3, dims=(H, W), modal_type='2D')
        # x = self.norm(x)
        out.append(x)

        return out, (H, W)

    def forward3D(self, x, t, m ,y):
        out = []
        ...
        for blk in self.block1:
            x = blk(x, dims=(D, W, H), modal_type='3D')
        cls_tokens = x[:, 0]
    ...
    def forward(self, x, modal_type, t, m, y):

        if modal_type == '2D':
            return self.forward2D(x, t, m, y)
        elif modal_type == '3D':
            return self.forward3D(x, t, m, y)
...
def encoder_tiny(norm_cfg2D='BN2', norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False, img_size2D=224,
    ...
    model = MiT_encoder(norm_cfg2D=norm_cfg2D, norm_cfg3D=norm_cfg3D, activation_cfg=activation_cfg,
                              ...
                              sr_ratios=[6, 4, 2, 1], qkv_bias=True,
...
if __name__ == "__main__":
    ...
    model = encoder_small(img_size3D=[64,96,96])
    model.train()
    out = model(x, '3D')
    print(len(out[0]))
