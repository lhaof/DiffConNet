...
from MiTnnu.network_architecture.DiT_1 import MiT_encoder as MiT_encoder
...
class MiT(nn.Module):
    ...
    def __init__(self, norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
        ...
        self.DecPosEmbed3D0 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed3D3.num_patches, embed_dims[0]))
        self.DecPosDrop3D0 = nn.Dropout(p=drop_rate)
        self.DecPosEmbed3D1 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed3D2.num_patches, embed_dims[1]))
        self.DecPosDrop3D1 = nn.Dropout(p=drop_rate)
        self.DecPosEmbed3D2 = nn.Parameter(
            torch.zeros(1, self.transformer.patch_embed3D1.num_patches, embed_dims[2]))
    ...
    def forward3D(self, inputs):
        ...
        x = x_encoder[-1].reshape(B, D, H, W, -1).permute(0, 4, 1, 2, 3).contiguous()
