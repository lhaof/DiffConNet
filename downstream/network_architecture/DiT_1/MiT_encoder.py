...
from MiTnnu.network_architecture.DiT_1 import Pacth_embeds as CNN_base

from MiTnnu.network_architecture.utils import trunc_normal_

from MiTnnu.network_architecture.DiT_1.DiT_models_0 import DiTBlock
...
class MiT_encoder(nn.Module):
    ...
    def __init__(self, norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
                 img_size3D=[16, 96, 96], in_chans=4,
        ...
        self.pos_embed3D1 = nn.Parameter(torch.zeros(1, self.patch_embed3D1.num_patches, embed_dims[0]))
        self.pos_drop3D1 = nn.Dropout(p=drop_rate)
        self.pos_embed3D2 = nn.Parameter(torch.zeros(1, self.patch_embed3D2.num_patches, embed_dims[1]))
        self.pos_drop3D2 = nn.Dropout(p=drop_rate)
        self.pos_embed3D3 = nn.Parameter(torch.zeros(1, self.patch_embed3D3.num_patches, embed_dims[2]))
        self.pos_drop3D3 = nn.Dropout(p=drop_rate)
        self.pos_embed3D4 = nn.Parameter(torch.zeros(1, self.patch_embed3D4.num_patches, embed_dims[3]))
        ...
        self.block1 = nn.ModuleList([DiTBlock(
            hidden_size=embed_dims[0], num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[0])
            for i in range(depths[0])])

        cur += depths[0]

        self.block2 = nn.ModuleList([DiTBlock(
            hidden_size=embed_dims[1], num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[1])
            for i in range(depths[1])])

        cur += depths[1]

        self.block3 = nn.ModuleList([DiTBlock(
            hidden_size=embed_dims[2], num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[2])
            for i in range(depths[2])])

        cur += depths[2]

        self.block4 = nn.ModuleList([DiTBlock(
            hidden_size=embed_dims[3], num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], sr_ratio=sr_ratios[3])
            for i in range(depths[3])])

        # self.cls_tokens1 = nn.Parameter(torch.zeros(1, 1, embed_dims[0]))
        # self.cls_tokens2 = nn.Linear(embed_dims[0], embed_dims[1])
        # self.cls_tokens2 = nn.Parameter(torch.zeros(1, 1, embed_dims[1]))
        # self.cls_tokens3 = nn.Linear(embed_dims[1], embed_dims[2])
        # self.cls_tokens3 = nn.Parameter(torch.zeros(1, 1, embed_dims[2]))
        # self.cls_tokens4 = nn.Linear(embed_dims[2], embed_dims[3])
        # self.cls_tokens4 = nn.Parameter(torch.zeros(1, 1, embed_dims[3]))
...
def encoder_tiny(norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
                 img_size3D=[16, 96, 96], **kwargs):
    model = MiT_encoder(norm_cfg3D=norm_cfg3D, activation_cfg=activation_cfg,
                              weight_std=weight_std,
...
def encoder_small(norm_cfg3D='BN3', activation_cfg='ReLU', weight_std=False,
                  img_size3D=[16, 96, 96], **kwargs):
    model = MiT_encoder(norm_cfg3D=norm_cfg3D, activation_cfg=activation_cfg,
                              weight_std=weight_std,
