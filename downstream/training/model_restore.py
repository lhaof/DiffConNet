...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    :return:
    """
    print("Loading model from ", pkl_file)
    info = load_pickle(pkl_file)
    init = info['init']
    name = info['name']
    search_in = join(MiTnnu.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="MiTnnu.training.network_training")
    ...
    # trainer = tr(plans_file=init[0], fold=init[1], output_folder=init[2], dataset_directory=init[3], batch_dice=init[4], stage=init[5], unpack_data=init[6],
    #                       deterministic=init[7], fp16=init[8], norm_cfg='IN', activation_cfg='LeakyReLU', epochs=300, pre_train=True, pre_path='/data2/kangluoyao/UniMiSS-code/UniMiSS/pretrain_weight/UniMiss_small.pth')

    trainer = tr(plans_file=init[0], fold=init[1], output_folder=init[2], dataset_directory=init[3], batch_dice=init[4], stage=init[5], unpack_data=init[6],
                          deterministic=init[7], fp16=init[8], norm_cfg='IN', activation_cfg='LeakyReLU', epochs=300, pre_train=False, pre_path='/data2/kangluoyao/UniMiSS-code/UniMiSS/pretrain_weight/UniMiss_small.pth')
