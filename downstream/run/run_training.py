...
from MiTnnu.run.default_configuration import get_default_configuration
from nnunet.paths import default_plans_identifier
from nnunet.training.cascade_stuff.predict_next_stage import predict_next_stage
from MiTnnu.training.network_training.nnUNetTrainer import nnUNetTrainer
from MiTnnu.training.network_training.nnUNetTrainerCascadeFullRes import nnUNetTrainerCascadeFullRes
from MiTnnu.training.network_training.nnUNetTrainerV2_CascadeFullRes import nnUNetTrainerV2CascadeFullRes
...
def main():
    ...
    parser.add_argument("-network_trainer", type=str, default='TrainerV2_BCV')
    ...
    parser.add_argument("-pre_train", default=False, help="use this if you want to load pretrained weights", required=False, action="store_true")
    parser.add_argument("-pre_path", type=str, default='xxx.pth', help='pretrained path')
    ...
    epochs = args.epochs
    # outpath = args.outpath + '_' + norm_cfg + '_' + activation_cfg + '_' + tag + '_' + str(split)
    outpath = args.outpath + '_' + norm_cfg + '_' + activation_cfg + '_' + str(split)
    ...
    trainer_class = get_default_configuration(outpath, network, task, network_trainer, plans_identifier, \
                                              search_in=(MiTnnu.__path__[0], "training", "network_training"), \
                                              base_module='MiTnnu.training.network_training')
    ...
    trainer = trainer_class(plans_file, fold, norm_cfg, activation_cfg, epochs, pre_train, pre_path, output_folder=output_folder_name,
                            dataset_directory=dataset_directory, batch_dice=batch_dice, stage=stage, unpack_data=decompress_data,
                            deterministic=deterministic, fp16=run_mixed_precision, split=split)
