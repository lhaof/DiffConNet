...
import DiT_nomod.utils as utils
import DiT_nomod.MiT as MiTs

from data_loader3D_brats_diffusion import Dataset3D
from data_loader2D import Dataset2D
from DiT_nomod.diffusion import create_diffusion
...
def get_args_parser():
    ...
    parser.add_argument('--img_size3D', default=[96,96,96], type=int,
                        help="""Size in pixels of input square 3D patches.""")

    parser.add_argument('--out_dim', default=48, type=int,
...
def train_func(args):
    ...
    # vae = AutoencoderKL.from_single_file("/data4/kangluoyao/UniMiSS-code/UniMiSS/pretrain_weight/vae/vae-ft-ema-560000-ema-pruned.ckpt").cuda()
    # vae = torch.load('/data4/kangluoyao/UniMiSS-code/UniMiSS/pretrain_weight/vae/vae-ft-ema-560000-ema-pruned.ckpt')['state_dict'].to(device)
    ...
    student = utils.ModuleWrapper(student)
    teacher = utils.ModuleWrapper(teacher)
    ...
    for epoch in range(start_epoch, args.epochs, interval*2):
        ...
        train_several_epoch(student, teacher, teacher_without_ddp, trainloss, data_loader3D, epoch, epoch+interval*2,
                        optimizer, lr_schedule, wd_schedule, momentum_schedule, fp16_scaler, '3D', args, diffusion)



def train_several_epoch(student, teacher, teacher_without_ddp, trainloss, data_loader, start_epoch, end_epoch,
                        optimizer, lr_schedule, wd_schedule, momentum_schedule, fp16_scaler, modal_type, args, diffusion):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if modal_type == '2D': 
            ...
            train_stats = train_one_epoch3D(student, teacher, teacher_without_ddp, trainloss,
                data_loader, optimizer, lr_schedule, wd_schedule, momentum_schedule,
                epoch, fp16_scaler, args, diffusion)
...
def train_one_epoch3D(student, teacher, teacher_without_ddp, trainloss, data_loader,
                    optimizer, lr_schedule, wd_schedule, momentum_schedule,epoch, fp16_scaler, args, diffusion):
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, subjects_batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        # update weight decay and learning rate according to their schedule
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            param_group["lr"] = lr_schedule[it-1]
            if i == 0:  # only the first group is regularized
                param_group["weight_decay"] = wd_schedule[it-1]

        # move 3D images to gpu
        images = [im.cuda(non_blocking=True) for im in subjects_batch[0]]

        #3D volume to 2D slices, sample 2D slices from 3D volume. Pick one of every two slices at random
        # image_slices1 = []
        # image_slices1= images[0][:,:,::8,:,:].view(-1, images[0].size()[-2], images[0].size()[-1]).unsqueeze(1)

        # randomly select 8 slices from 48 slices
        # slice_indices = np.random.choice(48, 8, replace=False)
        # image_slices1 = images[0][:,:,slice_indices,:,:].view(-1, images[0].size()[-2], images[0].size()[-1]).unsqueeze(1)

        # use all
        image_slices1 = images[0][:,:,:,:,:].view(-1, images[0].size()[-2], images[0].size()[-1]).unsqueeze(1)
        image_slices2 = images[1][:,:,:,:,:].view(-1, images[1].size()[-2], images[1].size()[-1]).unsqueeze(1)

        image_slices = torch.cat((image_slices1, image_slices2), 1)

        m = subjects_batch[1].cuda(non_blocking=True)
        y = subjects_batch[2].cuda(non_blocking=True)
        t = torch.randint(0, diffusion.num_timesteps, (image_slices1.shape[0],)).cuda()
        

        # image_slices1.append(images[0][:,1::2,:,:].view(-1, images[0].size()[-2], images[0].size()[-1]).unsqueeze(1))
        # image_slices2 = []
        # image_slices2.append(images[1][:,::8,:,:].view(-1, images[1].size()[-2], images[1].size()[-1]).unsqueeze(1))
        # image_slices2.append(images[1][:,1::2,:,:].view(-1, images[1].size()[-2], images[1].size()[-1]).unsqueeze(1))

        # teacher and student forward passes + compute 3D loss
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output = teacher(images[:2], t=t, m=m, y=y) # volume
            # print('teacher',teacher_output.shape)
            
            student_output = student(images, t=t, m=m, y=y) # volume
            # print('student', student_output.shape)

            volume_loss = trainloss(student_output, teacher_output, epoch)  # volume loss only

            # 2D slices
            B1 = image_slices1.shape[0]
            B2 = images[0].shape[0]
            # t = torch.tensor([t] * B).cuda()
            m = m.repeat(B1//B2,1).reshape(-1).cuda()
            y = y.repeat(B1//B2,1).reshape(-1).cuda()

            model_kwargs = dict(m=m, y=y)

            #     # Map input images to latent space + normalize latents:
            # image_slices1 = vae.encode(image_slices1).latent_dist.sample()
            # image_slices2 = vae.encode(image_slices2).latent_dist.sample()

            loss_dict_s = diffusion.training_losses(student, image_slices, t, model_kwargs)
            loss_dif_s = loss_dict_s["loss"].mean()

            # loss_dict_t = diffusion.training_losses(student, image_slices2, t, model_kwargs)
            # loss_dif_t = loss_dict_t["loss"].mean()
            
            # loss = volume_loss + 4 * loss_dif_s
            loss = volume_loss + 6 * loss_dif_s
        ...
        metric_logger.update(loss_diff_student=loss_dif_s.item())
        # metric_logger.update(loss_diff_teacher=loss_dif_t.item()) 
